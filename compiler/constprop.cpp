...
ConstPropagation::ConstPropagation(DFG* g, SymTab* t, vector<Var*>& paraVar)
    ...
    for (unsigned int i = 0; i < glbVars.size(); ++i) {
        ...
        if (!var->isBase())
            ...
            val = var->getVal();   //初始化的基本类型变量为const
...
double ConstPropagation::join(double left, double right) {
    if (left == NAC || right == NAC)
        return NAC;  //与NAC交并结果必是NAC
    else if (left == UNDEF)
        return right;  //与UNDEF交并结果不变
    else if (right == UNDEF)
        return left;
    else if (left == right)
...
void ConstPropagation::join(Block* block) {
    ...
    for (unsigned int i = 0; i < in.size();
        ...
        for (list<Block*>::iterator j = prevs.begin(); j != prevs.end();
             ++j) {  //处理每一个前驱基本块prev[j]
            val = join(val,
                       (*j)->outVals[i]);  //取出prev[j].out的每个元素与结果交并
        }
        in[i] = val;  //更新in[i]
    }
}
...
void ConstPropagation::translate(InterInst* inst, vector<double>& in,
    ...
    if (inst->isExpr()) {  //基本运算表达式x=?，计算新发现的传播值
        double tmp;        //存储临时值结果
        if (op == OP_AS || op == OP_NEG ||
            op == OP_NOT) {           //一元运算x=y x=-y x=!y
            if (arg1->index == -1) {  //参数不在值列表，必是常量
                if (arg1->isBase()) tmp = arg1->getVal();  //排除字符串类型常量
            } else
                tmp = in[arg1->index];         //获取in集合值
...
bool ConstPropagation::translate(Block* block) {
    ...
    for (list<InterInst*>::iterator i = block->insts.begin();
         i != block->insts.end(); ++i) {  //处理基本块的每一个指令
...
void ConstPropagation::analyse() {
    ...
    while (outChange) {     // B.out集合发生变化
        ...
        for (unsigned int i = 1; i < dfg->blocks.size(); ++i) {  //任意B!=Entry
            ...
            if (translate(
                    dfg->blocks[i]))  //传递函数执行时比较前后out集合是否有差别
                outChange = true;
        }
    }
}
    ...
    for (unsigned int j = 0; j < dfg->blocks.size(); ++j) {
        for (list<InterInst*>::iterator i = dfg->blocks[j]->insts.begin();
            ...
            if (inst->isExpr()) {  //处理每一个可能常量传播的指令
                ...
                if (rs != UNDEF &&
                    rs != NAC) {  //结果为常量，替换为result=c指令
                    // if(op==OP_AS||op==OP_NEG||op==OP_NOT){//一元运算时处理参数1
                    // 	if(arg1->index==-1)delete
                    // arg1;//参数1不在值列表，必是常量,删除！
                    // }
                    // else if(op>=OP_ADD&&op<=OP_OR){//二元运算时还要处理参数2
                    // 	if(arg1->index==-1)delete
                    // arg1;//参数1不在值列表，必是常量,删除！
                    // 	if(arg2->index==-1)delete
                    // arg2;//参数2不在值列表，必是常量,删除！
                    // }
                    Var* newVar = new Var((int)rs);  //计算的表达式结果
                    tab->addVar(newVar);             //添加到符号表
                    inst->replace(OP_AS, result,
                                  newVar);  //替换新的操作符与常量操作数
                    ...
                    if (op == OP_ADD) {      // z=0+y z=x+0
                        if (dol &&
                            left == 0) { /*if(arg1->index==-1)delete arg1;*/
                            newArg1 = arg2;
                        }
                        if (dor && right == 0) {
                        ...
                        if (dol &&
                            left == 1) { /*if(arg1->index==-1)delete arg1;*/
                            newArg1 = arg2;
                        }
                        if (dor && right == 1) {
                            newArg1 = arg1; /*if(arg2->index==-1)delete arg2;*/
                    ...
                    if (newArg1) {  //代数化简成功！
                        inst->replace(newOp, result, newArg1,
                                      newArg2);  //更换新的指令
                    } else {                     //没法化简，正常传播
                        if (dol) {               //传播左操作数
                        ...
                        inst->replace(op, result, newArg1,
                                      newArg2);  //更换传播的新的指令
                    }
                }
                ...
                if (arg1->index != -1) {      //不是常数
                    double rs =
                        inst->outVals[arg1->index];  //结果变量常量传播结果
...
/*
        条件跳转优化，同时进行不可达代码消除
        此时只需要处理JT JF
   两种情况，Jcond指令会在短路规则优化内产生，不影响表达式常量条件的处理
*/
    ...
    for (unsigned int j = 0; j < dfg->blocks.size(); ++j) {
        ...
        for (list<InterInst*>::iterator i = dfg->blocks[j]->insts.begin(),
            ...
            if (inst->isJcond()) {  //处理每一个可能常量传播的条件跳转指令，无条件跳转指令不用关心
                ...
                if (op == OP_JT) {    //条件真跳转
                    if (cond == 0) {  //条件不满足
                        inst->block->insts.remove(
                            inst);  //将该指令从基本块内删除
                        if (dfg->blocks[j + 1] !=
                            tar->block)  //目标块不是紧跟块
                            dfg->delLink(
                                inst->block,
                                tar->block);  //从DFG内递归解除指令所在块到目标块的关联
                    } else {
