...
#define LVAL_OPR                                                             \
    F(ASSIGN)                                                                \
    _(OR) _(AND) _(GT) _(GE) _(LT) _(LE) _(EQU) _(NEQU) _(ADD) _(SUB) _(MUL) \
        _(DIV) _(MOD) _(INC) _(DEC)
//右值运算
#define RVAL_OPR                                                              \
    F(OR)                                                                     \
    _(AND) _(GT) _(GE) _(LT) _(LE) _(EQU) _(NEQU) _(ADD) _(SUB) _(MUL) _(DIV) \
        _(MOD)
//语句
...
/*
        <type>				->
   rsv_int|rsv_char|rsv_bool|rsv_void
*/
Tag Parser::type() {
    Tag tmp = KW_INT;  //默认类型
    if (TYPE_FIRST) {
...
void Parser::def(bool ext, Tag t) {
    ...
    if (match(MUL)) {  //指针
        ...
        if (F(ID)) {  //变量 数组 函数
            ...
            recovery(F(SEMICON) _(COMMA) _(ASSIGN) _(LPAREN) _(LBRACK), ID_LOST,
                     ID_WRONG);
...
/*
        <idtail>			->	<varrdef><deflist>|lparen <para>
   rparen <funtail>
*/
...
/*
        <subprogram>	->
   <localdef><subprogram>|<statements><subprogram>|^
*/
...
/*
        <dowhilestat> -> 	rsv_do <block> rsv_while lparen<altexpr>rparen
   semicon <block>				->	<block>|<statement>
*/
...
/*
        <forstat> 		-> 	rsv_for lparen <forinit> semicon
   <altexpr> semicon <altexpr> rparen <block>
        <block>				->	<block>|<statement>
*/
void Parser::forstat() {
    symtab.enter();
...
/*
        <ifstat>			->	rsv_if
   lparen<expr>rparen<block><elsestat>
*/
void Parser::ifstat() {
    symtab.enter();
...
/*
        <switchstat>	-> 	rsv_switch lparen <expr> rparen lbrac <casestat>
   rbrac
*/
...
/*
        <casestat> 		-> 	rsv_case <caselabel> colon
   <subprogram><casestat> |rsv_default colon <subprogram>
*/
...
/*
        <elem>				->
   ident<idexpr>|lparen<expr>rparen|<literal>
*/
Var* Parser::elem() {
    Var* v = NULL;
    if (F(ID)) {  //变量，数组，函数调用
