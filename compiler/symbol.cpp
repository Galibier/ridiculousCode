...
/*
        获取VOID特殊变量
*/
Var* Var::getVoid() { return SymTab::voidVar; }
...
/*
        获取true变量
*/
Var* Var::getTrue() { return SymTab::one; }
...
bool Var::setInit() {
    ...
    // 	delete
    // init;//清除临时的字面变量，字符串和临时变量不清除（已经在符号表内）
    return false;
}
...
/*
        获取初始化变量数据
*/
Var* Var::getInitData() { return initData; }

/*
        获取extern
*/
bool Var::getExtern() { return externed; }

/*
        获取作用域路径
*/
vector<int>& Var::getPath() { return scopePath; }

/*
        获取类型
*/
Tag Var::getType() { return type; }
...
/*
        获取数组
*/
bool Var::getArray() { return isArray; }

/*
        设置指针变量
*/
void Var::setPointer(Var* p) { ptr = p; }

/*
        获取指针变量
*/
Var* Var::getPointer() { return ptr; }

/*
        获取字符指针内容
*/
string Var::getPtrVal() { return ptrVal; }

/*
        获取字符串常量内容
*/
string Var::getStrVal() { return strVal; }
...
/*
        设置变量的左值
*/
void Var::setLeft(bool lf) { isLeft = lf; }

/*
        获取变量的左值
*/
bool Var::getLeft() { return isLeft; }

/*
        设置栈帧偏移
*/
void Var::setOffset(int off) { offset = off; }

/*
        获取栈帧偏移
*/
int Var::getOffset() { return offset; }

/*
        获取变量大小
*/
int Var::getSize() { return size; }

/*
        是数字
*/
bool Var::isVoid() { return type == KW_VOID; }

/*
        是基本类型
*/
bool Var::isBase() { return !isArray && !isPtr; }

/*
        是引用类型
*/
bool Var::isRef() { return !!ptr; }

/*
        是否初始化
*/
bool Var::unInit() { return !inited; }

/*
        是否是常量
*/
bool Var::notConst() { return !literal; }

/*
        获取常量值
*/
int Var::getVal() { return intVal; }

/*
                是基本类型常量（字符串除外），没有存储在符号表，需要单独内存管理
*/
bool Var::isLiteral() { return this->literal && isBase(); }
...
Fun::Fun(bool ext, Tag t, string n, vector<Var*>& paraList) {
    ...
    paraVar = paraList;
    curEsp = Plat::
        stackBase;  //没有执行寄存器分配前，不需要保存现场，栈帧基址不需要修正。
    maxDepth = Plat::stackBase;  //防止没有定义局部变量导致最大值出错。
    //保存现场和恢复现场有函数内部解决，因此参数偏移不需要修正！
    for (int i = 0, argOff = 4; i < paraVar.size();
         i++, argOff += 4) {  //初始化参数变量地址从左到右，参数进栈从右到左
...
bool Fun::match(Fun* f) {
    ...
    if (paraVar.size() != f->paraVar.size()) return false;
    int len = paraVar.size();
    for (int i = 0; i < len; i++) {
        if (GenIR::typeCheck(paraVar[i], f->paraVar[i])) {  //类型兼容
            if (paraVar[i]->getType() !=
                f->paraVar[i]->getType()) {       //但是不完全匹配
...
/*
        添加一条中间代码
*/
void Fun::addInst(InterInst* inst) { interCode.addInst(inst); }

/*
        设置函数返回点
*/
void Fun::setReturnPoint(InterInst* inst) { returnPoint = inst; }

/*
        获取函数返回点
*/
InterInst* Fun::getReturnPoint() { return returnPoint; }

/*
        进入一个新的作用域
*/
void Fun::enterScope() { scopeEsp.push_back(0); }
...
/*
        设置extern
*/
void Fun::setExtern(bool ext) { externed = ext; }

/*
        获取extern
*/
bool Fun::getExtern() { return externed; }

Tag Fun::getType() { return type; }
...
/*
        获取参数列表，用于为参数生成加载代码
*/
vector<Var*>& Fun::getParaVar() { return paraVar; }
...
void Fun::genAsm(FILE* file) {
    ...
    if (Args::opt) {  //经过优化
        for (list<InterInst*>::iterator it = optCode.begin();
             it != optCode.end(); ++it) {
            code.push_back(*it);
        }
    } else {  //未优化，将中间代码导出
...
/*
        获取最大栈帧深度
*/
int Fun::getMaxDep() { return maxDepth; }
...
/*
        函数栈帧被重新定位了？
*/
bool Fun::isRelocated() { return relocated; }
