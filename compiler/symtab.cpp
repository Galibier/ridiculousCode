...
SymTab::~SymTab() {
    ...
    //清除变量
    hash_map<string, vector<Var*>*, string_hash>::iterator varIt,
        varEnd = varTab.end();
    ...
    for (strIt = strTab.begin(); strIt != strEnd; ++strIt) delete strIt->second;
}

/*
        设置中间代码生成器
*/
void SymTab::setIr(GenIR* ir) { this->ir = ir; }

/*
        获取scopePath
*/
vector<int>& SymTab::getScopePath() { return scopePath; }
...
void SymTab::addVar(Var* var) {
    if (varTab.find(var->getName()) == varTab.end()) {  //没有该名字的变量
        ...
        for (i = 0; i < list.size(); i++)
            if (list[i]->getPath().back() ==
                var->getPath().back())  //在一个作用域，冲突！
                break;
...
/*
        添加一个字符串常量
*/
void SymTab::addStr(Var* v) { strTab[v->getName()] = v; }
...
/*
        获取当前分析的函数
*/
Fun* SymTab::getCurFun() { return curFun; }
...
void SymTab::toString() {
    ...
    for (strIt = strTab.begin(); strIt != strEnd; ++strIt)
        printf("%s=%s\n", strIt->second->getName().c_str(),
               strIt->second->getStrVal().c_str());
    printf("----------函数表----------\n");
    for (int i = 0; i < funList.size(); i++) {
