...
void DFG::linkBlocks() {
    ...
    for (unsigned int i = 1; i < blocks.size(); ++i) {  //前驱关系
        InterInst* last =
            blocks[i - 1]->insts.back();  //前个基本块的最后一条指令
        if (!last->isJmp())  //不是直接跳转，可能顺序执行
    ...
    for (unsigned int i = 0; i < blocks.size(); ++i) {  //跳转关系
        ...
        if (last->isJmp() || last->isJcond()) {  //（直接/条件）跳转
            blocks[i]->succs.push_back(
                last->getTarget()->block);  //跳转目标块为后继
...
bool DFG::reachable(Block* block) {
    ...
    //若有前驱,测试每个前驱
    for (list<Block*>::iterator i = block->prevs.begin();
         i != block->prevs.end(); ++i) {
        Block* prev = *i;        //每个前驱
...
void DFG::release(Block* block) {
    ...
    if (!reachable(block)) {  //确定为不可达代码
        ...
        //先删除后继与当前块的关系
        for (list<Block*>::iterator i = delList.begin(); i != delList.end();
             ++i) {
