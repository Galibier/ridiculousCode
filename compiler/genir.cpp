...
Var* GenIR::genOr(Var* lval, Var* rval) {
    ...
    symtab.addVar(tmp);
    symtab.addInst(
        new InterInst(OP_OR, tmp, lval, rval));  //中间代码tmp=lval||rval
    return tmp;
}
...
Var* GenIR::genAnd(Var* lval, Var* rval) {
    ...
    symtab.addVar(tmp);
    symtab.addInst(
        new InterInst(OP_AND, tmp, lval, rval));  //中间代码tmp=lval&&rval
    return tmp;
}
...
Var* GenIR::genGt(Var* lval, Var* rval) {
    ...
    symtab.addVar(tmp);
    symtab.addInst(
        new InterInst(OP_GT, tmp, lval, rval));  //中间代码tmp=lval>rval
    return tmp;
}
...
Var* GenIR::genGe(Var* lval, Var* rval) {
    ...
    symtab.addVar(tmp);
    symtab.addInst(
        new InterInst(OP_GE, tmp, lval, rval));  //中间代码tmp=lval>=rval
    return tmp;
}
...
Var* GenIR::genLt(Var* lval, Var* rval) {
    ...
    symtab.addVar(tmp);
    symtab.addInst(
        new InterInst(OP_LT, tmp, lval, rval));  //中间代码tmp=lval<rval
    return tmp;
}
...
Var* GenIR::genLe(Var* lval, Var* rval) {
    ...
    symtab.addVar(tmp);
    symtab.addInst(
        new InterInst(OP_LE, tmp, lval, rval));  //中间代码tmp=lval<=rval
    return tmp;
}
...
Var* GenIR::genEqu(Var* lval, Var* rval) {
    ...
    symtab.addVar(tmp);
    symtab.addInst(
        new InterInst(OP_EQU, tmp, lval, rval));  //中间代码tmp=lval==rval
    return tmp;
}
...
Var* GenIR::genNequ(Var* lval, Var* rval) {
    ...
    symtab.addVar(tmp);
    symtab.addInst(
        new InterInst(OP_NE, tmp, lval, rval));  //中间代码tmp=lval!=rval
    return tmp;
}
...
Var* GenIR::genAdd(Var* lval, Var* rval) {
    ...
    symtab.addVar(tmp);
    symtab.addInst(
        new InterInst(OP_ADD, tmp, lval, rval));  //中间代码tmp=lval+rval
    return tmp;
}
...
Var* GenIR::genSub(Var* lval, Var* rval) {
    ...
    symtab.addVar(tmp);
    symtab.addInst(
        new InterInst(OP_SUB, tmp, lval, rval));  //中间代码tmp=lval-rval
    return tmp;
}
...
Var* GenIR::genMul(Var* lval, Var* rval) {
    ...
    symtab.addVar(tmp);
    symtab.addInst(
        new InterInst(OP_MUL, tmp, lval, rval));  //中间代码tmp=lval*rval
    return tmp;
}
...
Var* GenIR::genDiv(Var* lval, Var* rval) {
    ...
    symtab.addVar(tmp);
    symtab.addInst(
        new InterInst(OP_DIV, tmp, lval, rval));  //中间代码tmp=lval/rval
    return tmp;
}
...
Var* GenIR::genMod(Var* lval, Var* rval) {
    ...
    symtab.addVar(tmp);
    symtab.addInst(
        new InterInst(OP_MOD, tmp, lval, rval));  //中间代码tmp=lval%rval
    return tmp;
}
...
Var* GenIR::genIncL(Var* val) {
    ...
    symtab.addInst(
        new InterInst(OP_ADD, val, val, Var::getStep(val)));  //中间代码++val
    return val;
}
...
Var* GenIR::genDecL(Var* val) {
    ...
    symtab.addInst(
        new InterInst(OP_SUB, val, val, Var::getStep(val)));  //中间代码--val
    return val;
}
...
Var* GenIR::genLea(Var* val) {
    ...
    if (val->isRef())              //类似&*p运算
        ...
        Var* tmp = new Var(symtab.getScopePath(), val->getType(),
                           true);  //产生局部变量tmp
...
Var* GenIR::genIncR(Var* val) {
    ...
    symtab.addInst(
        new InterInst(OP_ADD, val, val, Var::getStep(val)));  //中间代码val++
    return tmp;
}
...
/*
        产生for条件开始部分
*/
void GenIR::genForCondBegin(Var* cond, InterInst*& _step, InterInst*& _block,
                            InterInst* _exit) {
...
/*
        产生if尾部
*/
void GenIR::genIfTail(InterInst*& _else) { symtab.addInst(_else); }
...
/*
        产生else尾部
*/
void GenIR::genElseTail(InterInst*& _exit) { symtab.addInst(_exit); }
...
void GenIR::genCaseHead(Var* cond, Var* lb, InterInst*& _case_exit) {
    ...
    if (lb)
        symtab.addInst(new InterInst(OP_JNE, _case_exit, cond,
                                     lb));  // if(cond!=lb)goto _case_exit
}
...
void GenIR::genCaseTail(InterInst* _case_exit) {
    ...
    // symtab.addInst(new InterInst(OP_JMP,_case_exit_append));//goto
    // _case_exit_append symtab.addInst(_case_exit);//添加case的exit标签
...
void GenIR::genReturn(Var* ret) {
    ...
    if (ret->isVoid() && fun->getType() != KW_VOID ||
        ret->isBase() && fun->getType() == KW_VOID) {  //类型不兼容
    ...
    if (ret->isVoid())
        symtab.addInst(
            new InterInst(OP_RET, returnPoint));  // return returnPoint
    else {
        if (ret->isRef()) ret = genAssign(ret);  //处理ret是*p情况
        symtab.addInst(new InterInst(OP_RETV, returnPoint,
                                     ret));  // return returnPoint ret
    }
}
...
bool GenIR::genVarInit(Var* var) {
    ...
    if (var->setInit())                          //初始化语句
        genTwoOp(var, ASSIGN,
                 var->getInitData());  //产生赋值表达式语句 name=init->name
    return 1;
}
...
void GenIR::genFunTail(Fun* function) {
    symtab.addInst(
        function->getReturnPoint());  //添加函数返回点，return的目的标号
