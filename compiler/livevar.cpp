...
LiveVar::LiveVar(DFG* g, SymTab* t, vector<Var*>& paraVar) : dfg(g), tab(t) {
    ...
    for (list<InterInst*>::iterator i = optCode.begin(); i != optCode.end();
        ...
        if (op >= OP_AS && op <= OP_LEA) {                  //常规运算
            ...
            if (arg2) inst->liveInfo.use.set(arg2->index);  //使用arg2
            if (rs != arg1 && rs != arg2)
...
bool LiveVar::translate(Block* block) {
    ...
    for (list<InterInst*>::reverse_iterator i = block->insts.rbegin();
        ...
        out = tmp;                      //设置指令的out
        in = inst->liveInfo.use |
             (out - inst->liveInfo.def);  //指令级传递函数in=f(out)
...
void LiveVar::analyse() {
    ...
    dfg->blocks[dfg->blocks.size() - 1]->liveInfo.out =
        E;  //初始化边界集合Exit.out=E
    translate(
        dfg->blocks[dfg->blocks.size() - 1]);  //传播给内部指令——其实就是exit
    //正常的数据流分析
    dfg->blocks[dfg->blocks.size() - 1]->liveInfo.in =
        E;  //初始化边界集合Exit.in=E
