...
void Error::synError(int code, Token *t) {
    ...
    if (code % 2 == 0)  // lost
        printf("%s<第%d行> 语法错误 : 在 %s 之前丢失 %s .\n",
               scanner->getFile(), scanner->getLine(), t->toString().c_str(),
               synErrorTable[code / 2]);
    else  // wrong
...
void Error::semWarn(int code, string name) {
    //语义警告信息串
    static const char *semWarnTable[] = {"函数参数列表类型冲突",  //附加名称信息
                                         "函数返回值类型不精确匹配"};
    warnNum++;
...
/*
        获取错误个数
*/
int Error::getErrorNum() { return errorNum; }

/*
        获取警告个数
*/
int Error::getWarnNum() { return warnNum; }
