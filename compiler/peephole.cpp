...
Window::Window(int sz, list<Arm*>& cd) : size(sz), code(cd) {
    int i = 0;
    for (pos = code.begin(); pos != code.end() && i < size;
         pos++, i++) {             //还有指令
        Arm* inst = *pos;          //取出指令
...
bool Window::match_double() {
    ...
    } else if ((op1 == "ldr" || op1 == "ldrb") && op2 == "mov" &&
               rs1 == a21) {  // ldr mov合并
        inst1.setDead();
    ...
    } else if ((op2 == "ldr" || op2 == "ldrb" || op2 == "str" ||
                op2 == "strb") &&
               op1 == "mov" && a21 == "[" + rs1 + "]" &&
               Plat::isReg(a11)) {  // mov合并到[r8]内存
        inst1.setDead();
        inst2.replace(op2, rs2, "[" + a11 + "]");
    } else if (op1 == "mov" && op2 == "cmp" &&
               rs1 == a21) {  // mov cmp合并比较数
        inst1.setDead();
        inst2.replace("cmp", rs2, a11);
    } else if (op1 == "mov" && op2 == "cmp" && rs1 == rs2 &&
               Plat::isReg(a11)) {  // mov cmp合并被比较数
        inst1.setDead();
    ...
    } else if (op1 == "mov" && op2 == "stmfd" && Plat::isReg(a11) &&
               a21 == "{" + rs1 + "}") {  // mov入栈合并
        inst1.setDead();
        inst2.replace(op2, rs2, "{" + a11 + "}");
...
bool Window::match_triple() {
    ...
    bool flag = true;
    if ((op1 == "str" && op3 == "ldr" || op1 == "strb" && op3 == "ldrb") &&
        op2 == "" && rs1 == rs3 && a11 == a31) {  //重复的str ldr
        inst1.setDead();                          //消除ldr，指令后移
        inst2.replace(op1, rs1, a11);
        inst3.replace("");
    } else if (!op1.find("mov") && op1.size() > 3 && !op2.find("mov") &&
               op2.size() > 3 && op3 == "mov" && rs1 == rs2 &&
               rs2 == a31) {  //比较结果合并mov
