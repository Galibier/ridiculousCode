...
/*
        0个操作数
*/
void ILoc::inst(string op, string rs) { emit(op, rs); }

/*
        一个操作数
*/
void ILoc::inst(string op, string rs, string arg1) { emit(op, rs, arg1); }
...
void ILoc::ldr_base(string rsReg, string base, int disp, bool isChar) {
    ...
    base = "[" + base + "]";  //内存寻址
    emit(isChar ? "ldrb" : "ldr", rsReg,
         base);  // ldr r8,[fp,#-16] ldr r8,[fp,r8]
}

/*
        基址寻址 str r0,[fp,#100]
*/
void ILoc::str_base(string srcReg, string base, int disp, string tmpReg,
                    bool isChar) {
    if (Plat::isDisp(disp)) {                 //有效的偏移常量
        if (disp) base += "," + toStr(disp);  //[fp,#-16] [fp]
    } else {
    ...
    base = "[" + base + "]";  //内存寻址
    emit(isChar ? "strb" : "str", srcReg,
         base);  // str r8,[fp,#-16] str r8,[fp,r9]
}
...
void ILoc::init_var(Var* var, string initReg, string tmpReg) {
    if (!var->unInit()) {                     //变量初始化了
        if (var->isBase())                    //基本类型初始化 100 'a'
            ...
            ldr_lb(initReg,
                   var->getPtrVal());  // ldr r8,=.L1, .L1 byte "1234567"
        //初始值已经加载完成！一般在r8
...
void ILoc::ldr_var(string rsReg, Var* var) {
    ...
    if (var->notConst()) {        //非常量
        ...
        if (id != -1)  //寄存器变量
            emit("mov", rsReg,
                 Plat::regName[id]);  // mov r8,r2 | 这里有优化空间——消除r8
        else {                        //所有定义的变量和数组
...
void ILoc::str_var(string srcReg, Var* var, string tmpReg) {
    ...
    bool isChar = var->isChar();  //是否是字符变量，需要类型转换
    if (id != -1) {               //寄存器变量
        emit("mov", Plat::regName[id],
             srcReg);  // mov r2,r8 | 这里有优化空间——消除r8
    } else {           //所有定义的变量,不可能是数组！
        int off = var->getOffset();  //栈帧偏移
        bool isGlb = !off;  //变量偏移，0表示全局，其他表示局部
        if (isGlb) {        //全局符号
            ldr_lb(tmpReg,
                   var->getName());  // ldr r9,=glb 地址寄存器加载变量地址
...
void ILoc::allocStack(Fun* fun, string tmpReg) {
    // int
    // base=(fun->isRelocated())?(Plat::stackBase_protect):(Plat::stackBase);
...
void ILoc::ldr_args(Fun* fun) {
    ...
    for (unsigned int i = 0; i < args.size(); i++) {
        Var* arg = args[i];
        if (arg->regId !=
            -1) {  //参数在寄存器，需要加载,不用理会是否是字符，统一4字节加载
            ldr_base(Plat::regName[arg->regId], "fp", arg->getOffset(),
                     false);  // ldr r0,[fp,#4]
        }
    }
}
...
/*
        比较
*/
void ILoc::cmp(string cmp, string cmpnot, string rsReg, string reg1,
               string reg2) {
