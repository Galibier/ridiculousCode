...
class GenIR {
   ...
   public:
    ...
    void genDoWhileTail(Var* cond, InterInst* _do,
                        InterInst* _exit);                 // do-while尾部
    void genForHead(InterInst*& _for, InterInst*& _exit);  // for循环头部
    void genForCondBegin(Var* cond, InterInst*& _step, InterInst*& _block,
                         InterInst* _exit);  // for条件开始
    void genForCondEnd(InterInst* _for,
                       InterInst* _block);  // for循环条件结束部分
