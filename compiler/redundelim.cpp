...
/*
        获取表达式的临时结果变量
*/
Var* Expr::getNewRs() { return newRs; }
...
/*
        表达式使用了变量v
*/
bool Expr::use(Var* v) { return arg1 == v || arg2 == v; }
...
RedundElim::RedundElim(DFG* g, SymTab* tab) : dfg(g), symtab(tab) {
    ...
    for (list<InterInst*>::iterator i = optCode.begin(); i != optCode.end();
        ...
        if (op >= OP_ADD && op <= OP_GET &&
            op != OP_SET) {                //所有的可能使用的表达式
    ...
    for (list<InterInst*>::iterator i = optCode.begin(); i != optCode.end();
        ...
        Var* arg1 = inst->getArg1();  //获取操作数1
        Var* arg2 = inst->getArg2();  //获取操作数2
        if (op >= OP_AS && op <= OP_GET &&
            op != OP_SET) {     //所有的可能使用的表达式
            if (op == OP_AS) {  //赋值语句特殊处理
                for (unsigned int i = 0; i < exprList.size(); ++i) {
                    if (exprList[i].use(rs))
                        inst->e_kill.set(i);  //设定杀死集合
                }
                continue;
            }
...
bool RedundElim::translate_anticipated(Block* block) {
    ...
    //逆序处理基本块的每一个指令
    for (list<InterInst*>::reverse_iterator i = block->insts.rbegin();
         i != block->insts.rend(); ++i) {
...
void RedundElim::analyse_anticipated() {
    ...
    */
    ...
    translate_anticipated(
        dfg->blocks[dfg->blocks.size() - 1]);  //将信息传递到指令上
    //正常的数据流分析
    dfg->blocks[dfg->blocks.size() - 1]->info.anticipated.in =
        E;  //初始化边界集合Exit.in=E
    ...
    while (change) {     // B.in集合发生变化
        ...
        for (int i = dfg->blocks.size() - 2; i >= 0; --i) {  //逆序，任意B!=Exit
            ...
            if (translate_anticipated(
                    dfg->blocks[i]))  //传递函数执行时比较前后集合是否有差别
                change = true;
        }
    }
    // 	/*
...
bool RedundElim::translate_available(Block* block) {
    ...
    for (list<InterInst*>::iterator i = block->insts.begin();
        ...
        in = tmp;                             //设置指令的in
        out = (inst->info.anticipated.in | in) -
              inst->e_kill;  //指令级传递函数out=f(in)
...
bool RedundElim::translate_postponable(Block* block) {
    ...
    for (list<InterInst*>::iterator i = block->insts.begin();
        ...
        //计算earliest
        inst->info.earliest =
            inst->info.anticipated.in - inst->info.available.in;
        out =
            (inst->info.earliest | in) - inst->e_use;  //指令级传递函数out=f(in)
        tmp = out;  //用当前指令的out设置下次使用的in
    }
    bool flag = tmp != block->info.postponable.out;  //是否变化
    block->info.postponable.out = tmp;               //设定B.out集合
    block->info.earliest = block->info.anticipated.in -
                           block->info.available.in;  //计算块的earliest
    return flag;
}
...
/*
        被使用表达式传递函数:f(x)=(e_use(B) | x)-latest(B)
                其中，latest(B)=(earliest(B) | postponable(B).in) &
                        (e_use(B) | ~(&( earliest(B.succ[i]) |
   postponable(B.succ[i]).in )))
*/
bool RedundElim::translate_used(Block* block) {
    ...
    //如果不是最后一条指令，则上式简化为earliest(B.succ[i]) |
    //postponable(B.succ[i]).in
    Set succJoin = U;
    for (list<Block*>::iterator i = block->succs.begin();
         i != block->succs.end(); ++i) {
    ...
    for (list<InterInst*>::reverse_iterator i = block->insts.rbegin();
        ...
        //记录succJoin=earliest(B) |
        //postponable(B).in，作为下次指令的后继交集集合
...
void RedundElim::analyse_used() {
    ...
    */
    ...
    //正常的数据流分析
    dfg->blocks[dfg->blocks.size() - 1]->info.used.in =
        E;  //初始化边界集合Exit.in=E
    for (unsigned int i = 0; i < dfg->blocks.size() - 1; ++i) {
...
void RedundElim::elimate() {
    ...
    // //冗余删除操作
    // vector<Set>
    // delarList(exprList.size(),BU);//存储表达式插入点支配节点集合交集
    ...
    // 			delarList[i]=delarList[i] &
    // inst->block->info.dom.out;//插入点基本块支配节点集合交集
    // 		}
    // 	}
    // }
    // //公共前驱算法——最近公共支配节点
    ...
    for (list<InterInst*>::iterator i = optCode.begin(); i != optCode.end();
        ...
        for (int i = 0; i < exprList.size(); ++i) {  //处理所有的表达式
            if (insert.get(i)) {                     //表达式i需要插入
                //插入点
                InterInst* newInst =
                    exprList[i].genInst(symtab, dfg);  //创建插入指令
                list<InterInst*>& insts =
                    inst->block->insts;  //指令所在基本块的指令序列
    ...
    for (list<InterInst*>::iterator i = optCode.begin(); i != optCode.end();
        ...
        Set replace = inst->e_use & (~inst->info.latest |
                                     inst->info.used.out);  //计算替换集合

        if (replace != E) {
            // printf("替换");
            // inst->toString();
            // printf(" -> ");
            inst->replace(OP_AS, inst->getResult(),
                          exprList[replace.max()].getNewRs());
            // inst->toString();
        }
