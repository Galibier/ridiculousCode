...
CopyPropagation::CopyPropagation(DFG* g) : dfg(g) {
    ...
    for (list<InterInst*>::iterator i = optCode.begin(); i != optCode.end();
        ...
        Operator op = inst->getOp();  //获取操作符
        if (inst->unknown()) {        //取值运算 *p=x /
                                //函数调用<非指针参数应该是仅仅杀死全局变量，忽略>
            inst->copyInfo.kill = U;               //杀死所有表达式
        } else if (op >= OP_AS && op <= OP_GET) {  //其他所有的表达式
            for (unsigned int i = 0; i < copyExpr.size();
                 i++) {                     //扫描所有复写表达式
...
bool CopyPropagation::translate(Block* block) {
    ...
    for (list<InterInst*>::iterator i = block->insts.begin();
        ...
        in = tmp;                       //设置指令的in
        out = (in - inst->copyInfo.kill) |
              inst->copyInfo.gen;  //指令级传递函数out=f(in)
...
Var* CopyPropagation::__find(Set& in, Var* var, Var* src) {
    ...
    for (unsigned int i = 0; i < copyExpr.size(); i++) {  //扫描所有复写表达式
        if (in.get(i) &&
            var == copyExpr[i]->getResult()) {  //找到可替换复写表达式
...
/*
        递归检测var赋值的源头的封装函数
*/
Var* CopyPropagation::find(Set& in, Var* var) { __find(in, var, var); }
