...
void InterInst::toString() {
    ...
    switch (op) {
        ...
        // ");arg2->value();printf(" )goto %s", 	target->label.c_str());break;
        // case OP_JGE:printf("if( ");arg1->value();printf(" >=
        // ");arg2->value();printf(" )goto %s", 	target->label.c_str());break;
        // case OP_JL:printf("if( ");arg1->value();printf(" <
        // ");arg2->value();printf(" )goto %s", 	target->label.c_str());break;
        // case OP_JLE:printf("if( ");arg1->value();printf(" <=
        // ");arg2->value();printf(" )goto %s", 	target->label.c_str());break;
        // case OP_JE:printf("if( ");arg1->value();printf(" ==
        // ");arg2->value();printf(" )goto %s", 	target->label.c_str());break;
        case OP_JNE:
            printf("if( ");
            arg1->value();
...
/*
        是否条件转移指令JT,JF,Jcond
*/
bool InterInst::isJcond() { return op >= OP_JT && op <= OP_JNE; }
...
/*
        标记首指令
*/
void InterInst::setFirst() { first = true; }

/*
        是首指令
*/
bool InterInst::isFirst() { return first; }

/*
        是否是标签
*/
bool InterInst::isLb() { return label != ""; }
...
/*
        获取跳转指令的目标指令
*/
InterInst *InterInst::getTarget() { return target; }
...
/*
        设置第一个参数
*/
void InterInst::setArg1(Var *arg1) { this->arg1 = arg1; }

/*
        是否是声明
*/
bool InterInst::isDec() { return op == OP_DEC; }
...
/*
        获取标签
*/
string InterInst::getLabel() { return label; }

/*
        获取函数对象
*/
Fun *InterInst::getFun() { return fun; }
...
void InterCode::markFirst() {
    ...
    for (unsigned int i = 1; i < len - 1; ++i) {
        if (code[i]->isJmp() ||
            code[i]
                ->isJcond()) {  //（直接/条件）跳转指令目标和紧跟指令都是首指令
...
/*
        获取中间代码序列
*/
vector<InterInst *> &InterCode::getCode() { return code; }
