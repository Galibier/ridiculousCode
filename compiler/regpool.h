...
class RegPool {
   ...
   public:
    static int
    getReg();  //获取一个空闲的寄存器,返回合法的寄存器编号,无空闲寄存器返回-NOMOREREG
    static void putReg(
        int reg_num);  //返回一个寄存器,要求编号合法，最好与getReg配对使用
    static void putAll();   //放回所有寄存器
    static bool isEmpty();  //判断寄存器池是否为空
    static int inUse(
        unsigned int reg_id);  //判断某个寄存器是否正在使用，即可被申请
...
/*
        重新初始化寄存器位图
*/
void RegPool::putAll() { reg_used = R_USABLE; }
