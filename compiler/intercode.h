...
class InterInst {
   ...
   public:
    ...
    //构造
    InterInst(Operator op, Var *rs, Var *arg1,
              Var *arg2 = NULL);                       //一般运算指令
    InterInst(Operator op, Fun *fun, Var *rs = NULL);  //函数调用指令,ENTRY,EXIT
    InterInst(Operator op, Var *arg1 = NULL);          //参数进栈指令,NOP
    InterInst();                                       //产生唯一标号
    InterInst(Operator op, InterInst *tar, Var *arg1 = NULL,
              Var *arg2 = NULL);  //条件跳转指令,return
    void replace(Operator op, Var *rs, Var *arg1,
                 Var *arg2 = NULL);  //替换表达式指令信息，用于常量表达式处理
    void replace(Operator op, InterInst *tar, Var *arg1 = NULL,
                 Var *arg2 = NULL);  //替换跳转指令信息，条件跳转优化
    ~InterInst();                    //清理常量内存
